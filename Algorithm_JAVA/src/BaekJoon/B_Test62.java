...
//	첫째 줄부터 총 N개의 줄에 걸쳐 온라인 저지 회원을 나이 순, 나이가 같으면 가입한 순으로 한 줄에 한 명씩 나이와 이름을 공백으로 구분해 출력한다.
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < len; i++) {
			member.add(sc.next() + " " + sc.next());
		}
		for (int i = 0; i < len; i++) {
			for (int j = 0; j < len-1; j++) {
				int age1 = Integer.parseInt(member.get(j).split(" ")[0]);
				int age2 = Integer.parseInt(member.get(j+1).split(" ")[0]);
				if(age1 > age2) {
					String tmp = member.get(j+1);
					member.set(j+1, member.get(j));
					member.set(j, tmp);
				}
