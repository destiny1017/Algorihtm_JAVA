...
//	근무 시간이 h 시간 m 분 s 초이면 h, m, s의 순으로 공백으로 분리하여 출력하라.
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 3; i++) {
			...
			int ed[] = new int[3];
			for (int j = 0; j < 6; j++) {
				if(j < 3) st[j] = sc.nextInt();
				else ed[j-3] = sc.nextInt();
			}
			int stSec = (st[0] * 3600) + (st[1] * 60) + st[2];
			int edSec = (ed[0] * 3600) + (ed[1] * 60) + ed[2];
			int diff = edSec - stSec;
			int hour = diff / 3600;
